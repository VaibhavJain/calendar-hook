...
<c:if test="<%= enableComments %>">
	<liferay-ui:panel-container extended="<%= false %>" id="calendarCommentsPanelContainer" persistState="<%= true %>">
		...
		<liferay-ui:panel collapsible="<%= true %>" extended="<%= true %>" id="calendarAttendeesPanel" title="attendees">
			<%
				List<CalEventAttendee> attendees = CalEventAttendeeLocalServiceUtil.findCalEventAttendeesByEventId(event.getEventId());
			...
			<%		
					...
					User attendee = UserLocalServiceUtil.getUser(calEventAttendee.getUserId());	
			%>		
						<div class="box-view">
							<div class="contactName"><%=attendee.getLastName() +" "+attendee.getFirstName()  %></div>
						</div>
