...
<aui:form action="<%= editEventURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveEvent();" %>'>
	...
	<liferay-ui:panel-container cssClass="calendar-event-details" extended="<%= true %>" id="calendarEventDetailsPanelContainer" persistState="<%= true %>">
		...
		<liferay-ui:panel collapsible="<%= true %>"  id="calendarAttendesssPanel" 
		title="attendees" extended="<%= false %>">
			...
			<aui:fieldset>
				...
				<aui:column columnWidth="45">
						...
						<div id="elements" class="contactBox">
								<c:forEach items="${attendees}" var="attendee">
									<div id="${attendee.userId}">
										<div class="box">
											<div class="contactName">${attendee.lastName} ${attendee.firstName}</div>
											<a class="cross" ></a>
											<input name="attendeesUserId" type="hidden" value="${attendee.userId}"></input>
											<input name="attendeesEmailAddress" type="hidden" value="${attendee.emailAddress}"></input>
										</div>
...
<aui:script use="aui-base,aui-custom-autocomplete">
	
	var autoComplete = new A.AutoComplete({
                dataSource : '<portlet:resourceURL/>',
                dataSourceType : 'IO',
                contentBox: '#userAutocomplete',
                matchKey : 'fullName',
                schema : {
                    resultFields : [ 'userId','firstName','lastName', 'fullName','emailAddress']
                },
                uniqueName:'keyword',
		        schemaType:'json',
		        typeAhead: true
	...
	autoComplete.on('itemSelect',function(event,selectedItem) {
		var array = document.getElementsByName('attendeesUserId');
		
		if(!existsAttendee(array,selectedItem)) {
			
			var attendeeContainer = A.Node.create('<div></div>');
			attendeeContainer.attr('id',selectedItem.userId);
			
			var divAttendeeLabelCont = A.Node.create('<div class="box"></div>');
			var divAttendeeLabel = A.Node.create('<div class="contactName">'+selectedItem.lastName+' '+selectedItem.firstName+'</div>');
			
			
			var aImage = A.Node.create('<a class="cross"></a>');
			aImage.on('click',function(evt) {
				A.one('#'+selectedItem.userId).remove();
			});
			
			divAttendeeLabelCont.appendChild(divAttendeeLabel);
			divAttendeeLabelCont.appendChild(aImage);
			
			var userId = A.Node.create('<input name="attendeesUserId" type="hidden"></input>');
			userId.attr('value',selectedItem.userId);
			
			var emailAddress = A.Node.create('<input name="attendeesEmailAddress" type="hidden"></input>');
			emailAddress.attr('value',selectedItem.emailAddress);
			
			attendeeContainer.appendChild(divAttendeeLabelCont);
			attendeeContainer.appendChild(userId);
			attendeeContainer.appendChild(emailAddress);
			
			attendeeContainer.appendTo('#elements');
			
			autoComplete.inputNode.set('value',null);
		}
		
	});
	
	function existsAttendee(array,selectedItem) {
		for(var i=0;i < array.length;i++) {
			if(array[i].tagName == 'INPUT') {
				if(array[i].value == selectedItem.userId)
					return true;
			}
	...
	A.all('.cross').each(function(node,index) {
		node.on('click',function(em) {
			var attendeeId = node.next().val();
			A.one('#'+attendeeId).remove();
		});
	});
